...
function clamp(value, min, max) {
    ...
    fetch('data.json')
        ...
        .then(data => {
            loader.load('fonts/Cosmica.json', function (font) {
                entries = data.Sheet1.filter(entry => entry.DATE).reverse();
    ...
    function updateSceneWithScaleWidth(scaleWidth) {
        // This function updates positions and lengths based on the current scaleWidth
        // After updating, call renderer.render(scene, camera) to reflect changes

        // Recalculate line lengths and positions for red/blue lines if needed
